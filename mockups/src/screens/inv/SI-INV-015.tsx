 ...
 *                              entry in a busy Dispatch hub at end of service).
 ...
 *
 * Expected qty semantics: production received − dispatched (Dispatch context).
 * Role: Dispatch Staff. Dept: Dispatch.
 *
 * Cut-off countdown:
 *   Derived from `cutOffRegistry` cutOffTime (00:30 — cross-midnight) vs a fixed
 *   SIMULATED_NOW constant (NOT Date.now() or argless new Date()). `warning` tone
 *   ≤ 60 min remaining; `error` tone if past cut-off. SIMULATED_NOW_TIME = '23:40'
 *   → 50 min before the 00:30 cut-off — intentionally inside the warning zone so the
 *   cut-off warning banner and warning-tone countdown render by default.
 *   Screen is mobile-first (Tier 1 acceptance).
 *
 * Submit flow:
 ...
 *
 * Fixtures: closingInventory (ci-002, Dispatch, deptType='dispatch', yesterday's
 *   confirmed-with-variance entry used as CC-PREFILL reference). One line
 *   (mat-mutton) is seeded with an implausible counted qty (32 kg vs expected 15 kg,
 *   213%) to demo FR114. Today's TRN = CI-2026-00015 (next in dispatch sequence).
 *
 ...
 * 23:40 local time → 50 min remaining until 00:30 cut-off (cross-midnight;
 * ≤ 60 min warning window). This is intentionally inside the warning zone so
 * the cut-off warning banner and `text-warning` countdown render by default.
 * The missed-cut-off / error path (past 00:30) remains reachable by changing
 * this to e.g. '00:45'.
 * NOT Date.now() or argless new Date().
 */
const SIMULATED_NOW_TIME = '23:40' as const

/**
 * Today's dispatch closing TRN (next in sequence after the existing fixtures).
 * Used as the ciTrn for the active form header + AuditLink.
 */
const TODAY_CI_TRN = 'CI-2026-00015' as const

/**
 * Derive minutes remaining until cut-off.
 * Handles cross-midnight cut-offs (e.g. 00:30 = 50 min after 23:40).
 */
...
/**
 * ci-002 = yesterday's CONFIRMED(variance_flagged) Dispatch entry (same loc + dept).
...
/**
 * Cut-off for the Dispatch location (loc-dh-andheri → 00:30, cross-midnight).
 */
 ...
 * Seeded to produce one FR114-tripping line (mutton: heard "32" vs expected 15 — 213%).
 */
const SIMULATED_HEARD: Record<string, string> = {
  'mat-mutton':     '32',   // FR114-tripping: 32 vs expected 15 (213%)
  'mat-ghee':       '3',
  'mat-basmati-rice': '18',
}
...
interface LineState {
  ...
  uom: string
  /** Active counted qty input. Initialised to fixture seed. */
  countedQty: string
...
// The 'active' countedQty seeds are chosen to demo implausibility on mutton.
// ─────────────────────────────────────────────────────────────────────────────

/** Counts we're "entering now" (today's count session). */
const ACTIVE_SEED: Record<string, string> = {
  'mat-mutton':       '32',  // FR114-tripping seed (213% of expected 15)
  'mat-ghee':         '3',
  'mat-basmati-rice': '18',
}
...
export default function SiInv015() {
  ...
  // ── Cut-off countdown ─────────────────────────────────────────────────────
  const cutOffTime = CUT_OFF_ENTRY?.cutOffTime ?? '00:30'
      ...
      <div className="mx-auto max-w-[1440px] px-4 py-6 tablet:px-6 tablet:py-8 desktop:px-10 desktop:py-10">
        ...
        <div className="mt-1 flex flex-col gap-2 tablet:flex-row tablet:items-start tablet:justify-between">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm text-on-surface-variant">
              End-of-day physical count for the Dispatch hub. Expected qty = production received − dispatched.
        ...
        {submitState === 'submitted_ok' && (
          <div className="mt-4 flex items-center gap-2 rounded-sm bg-surface-container px-4 py-3">
            ...
            <AuditLink entityRef={TODAY_CI_TRN} className="ml-auto" />
          </div>
        )}
        ...
        {/* ── Section 1: Dispatch context header ───────────────────────── */}
        ...
        <div className="mb-6">
          ...
          <div className="rounded-sm bg-surface-container-low p-4">
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <TrnDisplay trn={TODAY_CI_TRN} />
            </div>

            {/* Expected qty formula note */}
            <div className="mt-2 flex items-center gap-1.5">
              <Package className="h-3 w-3 shrink-0 text-on-surface-variant" aria-hidden />
              <span className="text-[11px] text-on-surface-variant">
                Expected qty = production received − dispatched (Dispatch Staff role)
              </span>
            </div>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Prod rcvd − dispatched</TableHead>
                  ...
                  return (
                    <React.Fragment key={`frag-${line.materialId}-${idx}`}>
                      <TableRow>
                        ...
                        {/* Expected qty — prod received − dispatched (read-only) */}
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex justify-end">
            <AuditLink entityRef={TODAY_CI_TRN} label="Closing audit history" />
          </div>
        </div>
        ...
        <p className="mt-4 text-[11px] text-on-surface-variant">
          SI-INV-015 · Tier 1 hero · Group 3 · Phase 4 Epic 4 Arc (b)
        </p>
      </div>
    </div>
  )
