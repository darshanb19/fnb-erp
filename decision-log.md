...
## DL-058 — 2026-06-26 — Production hotfix: cockpit shell wiring + runtime same-origin API resolution

**Context:** Founder reported "the design system is not correctly implemented — all screens look wrong" on the live site. Investigation (curl of live HTML/CSS/JS, Playwright login + console/network capture, dist/git inspection) found the design tokens and individual screens were built correctly. "All screens look wrong" was **two wiring/deploy defects**, not a design problem:

1. **API base URL baked to localhost.** `apps/web/src/hooks/use-api-client.ts` hardcoded `import.meta.env.VITE_API_BASE_URL ?? 'http://localhost:3001'` — an **unconditional** localhost fallback, so even a clean Vercel build (no `VITE_API_BASE_URL`) shipped a bundle that pointed real visitors at `localhost:3001` → every data fetch `ERR_CONNECTION_REFUSED` → every screen empty. The intended source of truth `apps/web/src/lib/api-config.ts` was **unused**. Also discovered: in this toolchain `vite build` does **not** reliably set `import.meta.env.DEV=false`, so a `DEV`-flag-based guard is not trustworthy.
2. **Cockpit shell never mounted.** `AppShell` (DESIGN.md §5.1.5 dark-teal sidebar + §5.4 top bar) was fully built but `App.tsx` routed all 40 pages raw → no chrome anywhere; `/` was a dev link-list scaffold; AppShell's nav (had it been mounted) pointed at dead `/SI-XXX-NNN` routes.

**Decision:**
- Resolve the API base URL from the **runtime `window.location.hostname`** (in `api-config.ts`), independent of unreliable build-mode flags: same-origin (`''` → `/api`) on any deployed host; `localhost:3001` only when served from a local dev host; an explicit **non-local** `VITE_API_BASE_URL` override still wins. `use-api-client.ts` now imports `API_BASE_URL` from `api-config.ts` (single source of truth).
- Mount `AppShell` as a **single layout route** wrapping `RequireAuth` around all authenticated pages (`RequirePermission` guards preserved). Added `apps/web/src/lib/screen-routes.ts` (id→real-path map; **built screens only** — sidebar renders no dead links). Added **Sign out** to the shell footer; removed the "Phase 2c-S2 scaffold" footer label. Replaced the scaffold `/` with a **clean on-brand welcome home** (greeting + role/brand + quick-action cards) per founder choice.

**Verification (live):** Vercel auto-deploy from `main` push (commit `dd7f4e6` → deployment `fnb-cuko19fqu`, READY). On https://fnb-erp-smoky.vercel.app: shell wraps the welcome home **and** inner screens; sidebar nav resolves to real routes; `GET /api/v1/departments` and `/api/v1/broadcasts` both **200** same-origin (was localhost `ERR_CONNECTION_REFUSED`); **0 console errors**. typecheck + vite build clean.
