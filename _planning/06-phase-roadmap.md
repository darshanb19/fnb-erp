...
> Last updated: 2026-05-07 (Phase 4 Epic 1 MDM Arc (b) mockups ✅ DONE — new shell `CCDuplicateWarn` per DL-026 + 4 Tier 2 mockups SI-MDM-001/002/005/007 + Index-only stub SI-MDM-006 + SI-MDM-003 fix-back consuming the new shell on `phase-4/epic-1-mdm-arc-b-mockups`; spec-compliance review clean; 6,240 lines added across 10 files; **Arc (c) production frontend is the next entry point on a fresh chat against `phase-4/epic-1-mdm-arc-c-frontend`** once Arc (a) + Arc (b) both merge).
...
| **4** | **Epic implementation** | **🔄 IN PROGRESS** | Real working app, epic-by-epic per Master Spec §10 canonical order. Each epic = backend (Drizzle schema + services + API) + just-in-time mockups (Tier 1 Acceptance Tag for deferred Group 2 + Group 3 heroes; Tier 2 / Tier 3 / Index for the rest) + production frontend consuming foundation chrome from Phase 2c-scoped. Chrome-freeze review gate at every epic boundary. **Epic 1 MDM kickoff complete 2026-05-07** — brainstorming Q1–Q7 → DL-022 → DL-026; canonical implementation plan at `docs/superpowers/plans/2026-05-07-phase-4-epic-1-mdm-build.md` (3 arcs, ~34 tasks). **Epic 1 Arc (a) backend ✅ DONE 2026-05-07** — Drizzle schema (org + inventory partial + procurement vendors-only + audit_log carve-out per DL-028) + 7 services (orgService DL-022, productService DL-023 + DL-026, categoryService, vendorService §2.7, inventoryService.checkEnablement, companyService DL-024 edit-only, auditLogService) + 31 REST routes + 178 tests green (1 skipped — DL-013 trigger backstop deferred to Phase 3a follow-up); zero `any`; brand seed idempotent; CI workflow (typecheck + test) shipped; new micro-decisions DL-027 (brandedDb explicit scoped methods) + DL-028 (audit_log carve-out). **Epic 1 Arc (b) mockups ✅ DONE 2026-05-07** — new shell `CCDuplicateWarn` per DL-026 (token substitution from plan-named `status_pending` to canonical `status_overridden` documented in shell JSDoc; canonical 20 status palette holds, no DESIGN.md amendment needed) + 4 Tier 2 mockups (SI-MDM-001 Org Hierarchy with DL-022 helper-text + no-re-parenting affordance; SI-MDM-002 Department Register with sortable table + responsive card-collapse mobile pattern; SI-MDM-005 Vendor Master with §2.7 inline 3-state scope picker + scope-mutation Popover-as-dialog with mandatory ≥10-char reason + widening-free vs narrowing/lateral-narrowing narrative + CC-DUPLICATE-WARN consumer #1; SI-MDM-007 Company Reg + Fiscal Year with DL-024 edit-only — no "Create new brand" affordance, one-way "Mark setup complete" CTA) + 1 Index-only stub SI-MDM-006 Categories (zero interactive surface per DL-025) + SI-MDM-003 fix-back consuming `<CCDuplicateWarn />` on the create-form name input (CC-DUPLICATE-WARN consumer #2; mock similarity heuristic against `materials` fixture — pg_trgm runs in Arc (c)). All 4 Tier 2 screens carry the canonical "Inventory schema" footer Card surfacing 12 fields each; spec-compliance reviewer pass clean. Cumulative diff vs Arc (a): +6,240 lines across 10 files. Typecheck silent + vite build clean throughout. **Arc (c) production frontend is the next entry point** on a fresh chat against `phase-4/epic-1-mdm-arc-c-frontend` once Arc (a) + Arc (b) both merge. | `phase-4/epic-1-mdm-plan` (planning PR #13) → `phase-4/epic-1-mdm-arc-a-backend` (Arc (a) backend PR #14 — open) → `phase-4/epic-1-mdm-arc-b-mockups` (Arc (b) mockups PR — next to open, stacked on PR #14) → `phase-4/epic-1-mdm-arc-c-frontend` opens at Arc (c) start |
