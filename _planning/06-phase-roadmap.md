...
> Last updated: 2026-06-23 (Phase 4 Epics 1 MDM + 2 USR + 3 INF ✅ DONE; first production deployment 2026-06-22 per DL-042 — all-on-Vercel full-stack at fnb-erp-smoky.vercel.app; Epic 3 chrome-freeze gate closed 2026-06-23 with Fix-back A finishing the provisional Task C10 audit-link sweep; **Epic 4 INV is the next entry point on a fresh chat**. Historical detail below predates this update.)
> Prior update: 2026-05-07 (Phase 4 Epic 1 MDM ✅ DONE — Arc (c) production frontend shipped on `phase-4/epic-1-mdm-arc-c-frontend`: 13 commits, +29,119 lines across 97 files; apps/web from scratch; 7 production pages SI-MDM-001 through SI-MDM-007 consuming real Arc (a) services with TanStack Query + DL-029 dev-stub auth; chrome-freeze review signed off "Drift = listed; fix-back applied at SHA `34f41d4`"; 15/15 Playwright e2e green against fnberp_dev; one deferred gap (DL-026 third consumer) flagged for Epic 2 due to Arc (a) backend lock; **Epic 2 USR (User Management & Security) is the next entry point on a fresh chat** — same per-epic 3-arc structure, with Supabase Mumbai provisioning at Arc (a) kickoff per DL-029).
...
| **4** | **Epic implementation** | **🔄 IN PROGRESS — Epics 1 MDM + 2 USR + 3 INF ✅ DONE; app LIVE in production (DL-042, fnb-erp-smoky.vercel.app); Epic 4 INV is next** | Real working app, epic-by-epic per Master Spec §10 canonical order. Each epic = backend (Drizzle schema + services + API) + just-in-time mockups (Tier 1 Acceptance Tag for deferred Group 2 + Group 3 heroes; Tier 2 / Tier 3 / Index for the rest) + production frontend consuming foundation chrome from Phase 2c-scoped. Chrome-freeze review gate at every epic boundary. **Epic 1 MDM ✅ DONE 2026-05-07.** Three-arc cadence: Arc (a) backend (178 tests, 31 routes, 7 services, audit_log carve-out per DL-028, brandedDb explicit-scoped per DL-027); Arc (b) just-in-time mockups (CCDuplicateWarn shell + 4 Tier 2 + Index-only stub + SI-MDM-003 fix-back); Arc (c) production frontend (apps/web from scratch — Vite scaffold + one-time copy-port per DL-005 of 22 chrome shells + 14 shadcn primitives + tokens/utils/personas/screen-catalog/sample-data; api-client.ts with §17.5 error envelope + Zod parsing; TanStack Query key factory across 10 MDM resources; main.tsx provider stack; **DL-029** dev-stub auth (jose-based HS256 JWT minted in browser, Supabase Mumbai provisioning + real Auth deferred to Epic 2 USR per FR14 dependency); 7 production pages SI-MDM-001 through SI-MDM-007 with full CRUD + DL-022/023/024/026 surfaces preserved; 15/15 Playwright e2e green; chrome-freeze review signed off — single deferred gap is DL-026 third consumer (Categories) blocked by Arc (a) backend lock). **Epic 2 USR is the next entry point** — Arc (a) provisions Supabase Mumbai (cost-bearing — explicit user authorisation required at kickoff), replaces DL-029 dev-stub auth.ts with real `@supabase/supabase-js` while preserving the `useSession()` consumer surface (Epic 1 pages keep working untouched), builds SI-USR-001 login + role/RBAC matrix + Brand Owner self-creation flow with Superadmin approval (FR14 — note Epic 2 surface depends on Epic 3 Approval Engine for actual approval routing; defer the routing wiring to Epic 3). | Epic 1: `phase-4/epic-1-mdm-plan` (planning PR #13) → `phase-4/epic-1-mdm-arc-a-backend` (PR #14 merged) → `phase-4/epic-1-mdm-arc-b-mockups` (PR #15 merged) → `phase-4/epic-1-mdm-arc-c-frontend` (Arc (c) PR — open at Arc (c) close) |
